...
app.patch('/api/integration/:id', async (req, res) => {
  ...
  try {
    ...
    if (req.body.statusHistory) {
      integration.statusHistory = req.body.statusHistory;
      integration.changed('statusHistory', true);
    ...
    Object.keys(req.body).forEach(key => {
      if (allowedFields.includes(key)) {
        if (key === 'clientsdata_id' || key === 'client_id' || key === 'numberOfBots') {
          ...
          if (value === '' || value === null || value === undefined) {
            ...
            if (isNaN(numValue)) {
              ...
              integration[key] = numValue;
              console.log(`Set ${key} to ${numValue}`);
            }
          }
        } else {
    ...
    console.log('PATCH SUCCESS - numberOfBots:', integration.numberOfBots);
    ...
    console.error('PATCH ERROR:', error.name, error.message);
